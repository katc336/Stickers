...
import { useGetTeacherQuery, usePostNewUserProfileMutation } from '../../../../redux/api';
...
const UploadAvatar = () => {
    ...
    const { data, error: dataError, isLoading } = useGetTeacherQuery();
    if (isLoading) {
        return <div></div>
    }
    if (dataError) {
    ...
    const upLoadImage = (event) => {
        ...
        try {
            ...
            formData.append("cloud_name", CLOUD_NAME);// add key: "cloud_name", value: name of cloud (NOTE: deleted old demo: new name added and updated in .env)
            formData.append("upload_preset", UPLOAD_PRESET ); // add key: "upload_preset", value: preset (NOTE: deleted old demo: new name added and updated in .env)
